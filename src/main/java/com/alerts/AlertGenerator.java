...
public class AlertGenerator {
    ...
    public void evaluateSystolicPressure(Patient patient, PatientRecord lastUploaded){
        ...
        if ((currentValue > 180 || currentValue < 90)) triggerAlert(new Alert(String.valueOf(patient.getPatientId()), "Critical Threshold Alert pressure", lastUploaded.getTimestamp(), 1));
        ...
        if (currentValue<90) {
            ...
            if (found)
                triggerAlert(new Alert(String.valueOf(patient.getPatientId()), "Hypotensive Hypoxemia Alert", lastUploaded.getTimestamp(),1));
        }
    }
    public void evaluateDiastolicPressure(Patient patient, PatientRecord lastUploaded) {
        ...
        if ((currentValue > 120 || currentValue < 60))
            triggerAlert(new Alert(String.valueOf(patient.getPatientId()), "Critical Threshold Alert pressure", lastUploaded.getTimestamp(),1));
    }
    public void evaluateSaturation(Patient patient, PatientRecord lastUploaded) {
        double currentValue=lastUploaded.getMeasurementValue();

        if (currentValue<92) triggerAlert(new Alert(String.valueOf(patient.getPatientId()), "Low Saturation Alert", lastUploaded.getTimestamp(),0));
        ...
        } if (condition && lastUploaded.getMeasurementValue()-max>5) triggerAlert(new Alert(String.valueOf(patient.getPatientId()), "Saturation drop Alert", lastUploaded.getTimestamp(),1));
    }
    ...
    public void trendAlert(Patient patient, String typeValue){
        ...
        if (counter!=-1) return;
        if (values[1]-values[0]>10 && values[2]-values[1]>10) triggerAlert(new Alert(String.valueOf(patient.getPatientId()), "Trend Alert",record.get(record.size()-1).getTimestamp(),-1));
    }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            switch (current.getRecordType()) {
                case ("SystolicPressure"):
                    ...
                    if (systolicCheck[1]>2 && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Trend Alert", timestamp , -1));


                    if ((currentValue > 180 || currentValue < 90)  && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Critical Threshold Alert pressure", timestamp, 1));
                    lowSystolicPressure= currentValue < 90;
                    if (lowSaturation && lowSystolicPressure  && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Hypotensive Hypoxemia Alert", timestamp, 1));
                    break;
                case ("DiastolicPressure"):
                    ...
                    if (diastolicCheck[1]>2  && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Trend Alert", timestamp, -1));

                    if ((currentValue > 120 || currentValue < 60) && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Critical Threshold Alert pressure", timestamp, 1));
                    // implement trend alert
                    break;
                case ("Saturation"):
                    if (currentValue<92.0  && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Low Saturation Alert", timestamp, 0));
                    lowSaturation= currentValue<92;
                    if (lowSaturation && lowSystolicPressure  && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Hypotensive Hypoxemia Alert", timestamp, 1));
                    if (currentValue>max) max=currentValue;
                    if (max-currentValue>5 && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Rapid drop Alert", timestamp,1));
                    break;
                case ("ECG"):
                    if (averageECG!=0 && Math.abs(averageECG-currentValue)>anomaly  && i==list.size()-1) triggerAlert(new Alert(String.valueOf(patientID), "Abnormal data", timestamp, 0));
